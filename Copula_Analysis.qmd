...
# Cumulative
cum_loss_vine = sim_port_vine.to_numpy()
cum_loss_gauss = sim_port_gauss.to_numpy()
var_cum_vine, es_cum_vine = (
    np.quantile(cum_loss_vine, 0.99),
    cum_loss_vine[cum_loss_vine >= np.quantile(cum_loss_vine, 0.99)].mean(),
)
var_cum_gauss, es_cum_gauss = (
    np.quantile(cum_loss_gauss, 0.99),
    cum_loss_gauss[cum_loss_gauss >= np.quantile(cum_loss_gauss, 0.99)].mean(),
)

# 1-day anchor for the naive scaling rule
var_1d = var_cum_by_h_vine[0]
var_path_scaled = var_1d * np.sqrt(np.arange(1, 11))
var_cum_scaled = var_1d * np.sqrt(10)
...
ax = axes[0]
ax.plot(h_axis, var_cum_by_h_vine, "o-", color="steelblue", label="MC — Vine")
ax.plot(h_axis, var_cum_by_h_gauss, "s--", color="orange", label="MC — Gauss")
ax.plot(h_axis, var_path_scaled, "x:", color="black", label="√t scaling")
ax.set_xlabel("Horizon h (trading days)")
ax.set_ylabel("99% VaR of cumulative loss")
ax.set_title("Cumulative VaR by horizon")
ax.legend()
...
ax.axvline(var_cum_vine, color="steelblue", lw=2, label=f"VaR Vine  {var_cum_vine:.4f}")
ax.axvline(var_cum_gauss, color="orange", lw=2, label=f"VaR Gauss {var_cum_gauss:.4f}")
ax.axvline(
    var_cum_scaled,
    color="black",
    ls=":",
    lw=1.5,
    label=f"VaR √t   {var_cum_scaled:.4f}",
)
